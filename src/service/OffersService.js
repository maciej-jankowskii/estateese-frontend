...
class OffersService {
	...
	static async getAllOffers(token, page, pageSize) {
		try {
			const response = await axios.get(`${OffersService.BASE_URL}`, {
	...
	static async updateOffer(id, offerData, token) {
		try {
			const response = await axios.put(
				`${OffersService.BASE_URL}/update-offer/${id}`,
				offerData,
				{
					headers: { Authorization: `Bearer ${token}` },
				}
	...
	static async deleteOffer(id, token) {
		try {
			const respone = await axios.delete(
				`${OffersService.BASE_URL}/delete-offer/${id}`,
				{
					headers: { Authorization: `Bearer ${token}` },
				}
			);
	...
	static async markOfferAsSold(id, token) {
		try {
			const response = await axios.patch(
				`${OffersService.BASE_URL}/sold/${id}`,
				null,
				{
					headers: { Authorization: `Bearer ${token}` },
				}
