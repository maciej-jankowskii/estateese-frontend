...
class CommercialPropertyService {
	...
	static async getCommercialPropertyById(token, id) {
		try {
			const response = await axios.get(
				`${CommercialPropertyService.BASE_URL}/${id}`,
				{
					headers: { Authorization: `Bearer ${token}` },
				}
			);
	...
	static async addCommercialProperty(commercialData, token) {
		try {
			...
			if (error.response && error.response.data) {
				...
				throw new Error(
					"Adding the commercial property failed. Please try again"
				);
			}
		}
	}
	...
	static async updateCommercialProperty(id, commercialData, token) {
		try {
			const response = await axios.put(
				`${CommercialPropertyService.BASE_URL}/update-commercial/${id}`,
				commercialData,
				{
					headers: { Authorization: `Bearer ${token}` },
				}
			...
			if (error.response && error.response.data) {
				...
				throw new Error(
					"Updating the commercial property failed. Please try again"
				);
			}
		}
	}
	...
	static async deleteCommercialProperty(id, token) {
		try {
			const respone = await axios.delete(
				`${CommercialPropertyService.BASE_URL}/delete-commercial/${id}`,
				{
					headers: { Authorization: `Bearer ${token}` },
				}
			);
